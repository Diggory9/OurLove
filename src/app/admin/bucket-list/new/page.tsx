...
import { createBucketItem } from "@/lib/admin-api";

export default function NewBucketItemPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      await createBucketItem({ title, description, category, order });
      router.push("/admin/bucket-list");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Lỗi tạo mục");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Thêm kế hoạch mới</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-5 bg-white rounded-xl p-6 shadow-sm">
        ...
        <div>
          ...
          <input
            ...
            placeholder="Ví dụ: Đi du lịch Đà Lạt"
        ...
        <div>
          ...
          <input
            ...
            placeholder="Ví dụ: Du lịch, Ẩm thực, Trải nghiệm..."
            className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20"
          />
        </div>
